...
from .util import get_kls_full_path, update_forward_refs, shelling_type
...
def _get_pydantic_attrs(kls):
    for k, v in kls.model_fields.items():
        shelled_type = shelling_type(v.annotation)
...
def scan_and_store_required_fields(target):
    ...
    def walker(kls):
        ...
        if issubclass(kls, BaseModel):
            all_attrs = set(kls.model_fields.keys())
